...
import { Plus, Trash2, Settings, Loader2, Tag, Cpu, Users2, Activity, Mail, Eye, EyeOff, FlaskConical, CheckCircle, XCircle, ToggleLeft, ToggleRight } from "lucide-react";
      ...
      <div className="px-6 py-4">
        {options.length === 0 ? (
          ...
          <div className="space-y-1.5 mb-4">
            {options.map((opt) => (
              <div key={opt.id} className="flex items-center justify-between px-3 py-2.5 rounded-lg bg-slate-50 hover:bg-slate-100 border border-slate-100 group transition-colors">
                <div className="flex items-center gap-2">
                ...
                <button onClick={() => handleDelete(opt.id, opt.value)} disabled={deletingId === opt.id}
                  className="p-1.5 rounded-lg text-slate-300 hover:text-red-500 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all disabled:opacity-50" title={`Remove "${opt.value}"`}>
